...
module.exports = (email, password, branch, logAmount, ptr = false) => {
  return gulpCollect.list((fileVinyls, cb) => {
    let cbd = Q.defer();
    _gulpUploadVinylsAsModules(fileVinyls, cbd.makeNodeResolver(), email, password, branch, logAmount, ptr);
    return cbd.promise;
  });
};
...
function _gulpUploadVinylsAsModules(fileVinyls, cb, email, password, branch, logAmount, ptr) {
  let modules = {};
  ...
  let codepath = '/api/user/code';
  if (ptr) {
    codepath = '/ptr/api/user/code';
    gutil.log(`Uploading to PTR`);
  }
