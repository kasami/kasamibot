...
gulp.task('compile-bundled', gulp.series(gulp.parallel('clean'), function bundle() {
...
gulp.task('uploadptr', gulp.series('compile', function uploading() {
  if(buildConfig.branch) {
    return gulp.src('dist/' + buildTarget + '/*.js')
      .pipe(gulpRename((path) => path.extname = ''))
      .pipe(gulpScreepsUpload(config.user.email, config.user.password, buildConfig.branch, 0, true));
  } else {
    return gulp.src('dist/' + buildTarget + '/*.js')
      .pipe(gulp.dest(buildConfig.localPath));
  }
...
gulp.task('buildbot', gulp.series('compile', function uglifying() {
  return gulp.src('dist/' + buildTarget + '/main.js')
    /*.pipe(babel({
      presets: ['es2015']
    }))
    .pipe(uglify({
    ...
    .pipe(gulp.dest('dist/' + buildTarget + '/release'))*/
  }
));
...
gulp.task('buildptr', gulp.series('uploadptr', function buildDone(done) {
  gutil.log(gutil.colors.green('Build done'));
  return done();
}));

gulp.task('buildmin', gulp.series('buildbot', function buildDone(done) {
  gutil.log(gutil.colors.green('Build done'));
  return done();
}));
